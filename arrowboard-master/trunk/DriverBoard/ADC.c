...
// ADC0 channels for Board Rev L and above
#define ADC_CHANNEL_SYS_CURRENT     0  
#define ADC_CHANNEL_RSSI_VOLTAGE    1 

#define ADC_CHANNEL_PHOTOCELL       3
#define ADC_CHANNEL_12V_CURRENT     4  
#define ADC_CHANNEL_SYS_VOLTAGE     5  
...
static ADC_FILTER systemVoltageFilter;
...
{
	return systemVoltageFilter.filteredValue;
}
...
static DWORD ADCHWInit( DWORD ADC_Clk )
{
  ...
  PCONP |= (1 << 12);
	...
	// P0.23 = ADC0.0   P0.24 = ADC0.1  P0.26 = ADC0.3
	PINSEL1 |= ((1UL << 14) | (1UL << 16) | (1UL << 20));		
	// P1.30 = ADC0.4   P0.31 = ADC0.5
	PINSEL3 |= ((3UL << 28) | (3UL << 30));		
...
void ADCInit(int boardRev)
{
	...
	if(nBoardRev < 6)
    { 
        // pre revH boards had off-board ADC
        ADS7828Init();
	}
    
    initAdcFilter(&systemVoltageFilter);
...
void ADCDoWork()
{
    ...
    {	
        ...
        if(1 == ADC5IntDone)
        {
            ...
            systemVoltageFilter.filteredValue = adcFilter(&systemVoltageFilter, nValue);
        }
        ...
        // Determine which channel to start conversion
				switch( nChannelNum )
				{
          case ADC_CHANNEL_SYS_CURRENT:
            nChannelNum = ADC_CHANNEL_RSSI_VOLTAGE;
            break;
          case ADC_CHANNEL_RSSI_VOLTAGE:
            nChannelNum = ADC_CHANNEL_PHOTOCELL;
            break;
          case ADC_CHANNEL_PHOTOCELL:
            nChannelNum = ADC_CHANNEL_12V_CURRENT;
            break;
          case ADC_CHANNEL_12V_CURRENT:
            nChannelNum = ADC_CHANNEL_SYS_VOLTAGE;
            break;
          case ADC_CHANNEL_SYS_VOLTAGE:
            nChannelNum = ADC_CHANNEL_SYS_CURRENT;
            break;
          default:
            nChannelNum = ADC_CHANNEL_SYS_CURRENT;
            break;
...
// Supply voltage, battery, alternator, etc.
DWORD ADCGetVl()
{
    return ADC0Value[ADC_CHANNEL_SYS_VOLTAGE];
}
...
DWORD ADCGetPr()
{
    return ADC0Value[ADC_CHANNEL_PHOTOCELL];
}
